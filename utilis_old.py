...
#            print(line_b,p0)
            popt,pcov,infodict,errmsg,ier = leastsq(residuals,x0=p0,
                                    ...
                                    full_output=True)
            cen, flx = popt
            line_m = flx * interpolate.splev(line_x+cen,psf_rep) 
            line['data'].loc[dict(ax='mod')] = line_m
            if ier not in [1, 2, 3, 4]:
                print("Optimal parameters not found: " + errmsg)
                popt = np.full_like(p0,np.nan)
                pcov = None
...
#        dflux         = np.gradient(spec1d['flux'])#,dlambda)
    ...
    def plot_spectrum(self,order=None,nobackground=False,scale='wave',fit=False,
        ...
        '''
        def model_line(od,lid):
            line_x = lines['data'].sel(od=order,id=lid,ax='pix')
            cen    = lines['pars'].sel(od=order,id=lid,par='cen')
            flx    = lines['pars'].sel(od=order,id=lid,par='flx')
