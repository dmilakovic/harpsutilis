...
freqA = seriesA_2012['freq_gauss']
freqB = seriesB_2012['freq_gauss']
...
def temporal_fit(group1,group2,model='linear'):
    ...
    shift1 = group1.mean('3sigma')
    time2  = group2.mean('datetime')
    shift2 = group2.mean('3sigma')
...
for i,f in enumerate(['A','B']):
    ...
    for d, method in zip(data[f],methods):
        dtimes   = d['datetime'][10::10].values
        time_bins = hf.tuple_to_datetime(dtimes)
        time_groups = d.groupby_bins('datetime',time_bins)
        A_t, B_t, time1 = temporal_fit(time_groups['0'],time_groups['14'])
        
        x1 = (hf.tuple_to_datetime(d.values['datetime']) - \
              hf.tuple_to_datetime(d.values['datetime'][0])).astype(np.float64)
        time_plotter.axes[i].plot(x1,d['3sigma'].values[:,0],lw=0.4,
                         label="{0} uncorrected".format(f),marker='o')
        dc = d.correct_time((A_t,B_t),time1,copy=True)
        
        data_fibre.append(dc)
        
        time_plotter.axes[i].plot(x1,dc['3sigma'].values[:,0],lw=0.4,
