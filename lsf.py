...
class LSFModeller(object):
    ...
    def get_linelist(self):
        try:
            linelist = self._cache['linelist']
        except:
            cache    = io.mread_outfile(self.outfile,['linelist'])
            linelist = cache['linelist']
            self._cache['linelist'] = linelist
        return linelist
    def get_data(self):
        try:
            data = self._cache['data']
        except:
            data = io.mread_e2ds_data(self.e2dsfile)
...
def stack_lines1d(linelist,data,segnum,subnum,fittype='gauss'):
    ...
    for i in range(segnum):
        pixl = seglims[i]
        pixr = seglims[i+1]
        cut  = np.where((linelist['pixl']>=pixl)&(linelist['pixr']<=pixr))
        lines = linelist[cut]
