...
USAGE : 
    ...
    python gaps.py $path/to/settings/file.json 
...
#    filepath = '/Users/dmilakov/harps/dataprod/settings/series1_fibreA.json'
    ...
    print("TOTAL N OF LINES = {0:6.3f} M".format(np.size(linesarr)/1e6))
    fig,ax = hf.figure(1)
    ax[0].scatter(linesarr['gauss'][:,1],residarr['residual'],s=2)
...
def plot_all(bincen,binval,binstd,binlims,coeffs,fitres,vals,seglims,centers,residuals):
    print("NUMBER OF LINES = {0:6.3f} k".format(np.size(centers)/1e3))
...
if __name__=='__main__':
    ...
    parser.add_argument('file',type=str, require=True,
    ...
    parser.add_argument('-poly','-polyord',type=int,default=8,
                        help="Polynomial order")
    parser.add_argument('-b','-bins',type=int,default=8,
                        help="Number of bins in each 512px segment")
