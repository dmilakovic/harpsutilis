...
def solve(data):
    ...
    for order in orders:
        for seg in segments:
            epsf_x  = data['epsf'].sel(ax='x',od=order,seg=seg).dropna('pix')
            epsf_y  = data['epsf'].sel(ax='y',od=order,seg=seg).dropna('pix')
            splr    = splrep(epsf_x.values,epsf_y.values)
        for idx in midx:
            sg,sp,lid = idx
            line_x = data['line'].sel(ax='x',idx=idx,od=order).dropna('pix').values
            ...
            p0     = data['pars'].sel(idx=idx,od=order,val=['sft','flx']).values
            ...
            popt,pcov,infodict,errmsg,ie = leastsq(residuals,x0=p0,
                                                   ...
                                                   full_output=True)
            shift, peakflux = popt
            data['pars'].loc[dict(idx=idx,od=order,val=['sft','flx'])] = popt
            ...
            data['line'].loc[dict(idx=idx,od=order,ax='x',pix=line_pix)] += shift
...
#                    line_shifts   = 
                ...
                if first_iteration:
                    ...
                    cen,flux,shift = data['pars'].sel(od=order,idx=idx)
                peakflux = flux
                data['pars'].loc[dict(idx=idx,od=order)] = cen,flux,shift
...
#                        epsf_y0 = data['epsf'].sel(ax='y',seg=n,od=order).dropna('pix','all')
                    ...
                    ax[n].scatter(x_data.values,y_data.values,s=1,c='C{}'.format(j+1),marker='s')
    ...
    return data
def return_ePSF(manager,line_positions=None,line_fluxes=None,
    ...
    data0   = xr.Dataset({'line': (['od','pix','ax','idx'], np.full((nOrders,npix*N_sub,8,ndix),np.nan)),
                     ...
                     'pars': (['od','idx','val'], np.full((nOrders,ndix,3),np.nan)) },
                     coords={'od':orders, 
                             'idx':mdix, 
                             'pix':pixels,
                             'seg':np.arange(N_seg),
    ...
    fig,ax = h.get_fig_axes(9,alignment='grid',title='PSF iteration')
    while j < 5:
        ...
        for idx in midx:
            ...
            ax[sg].scatter(data_x,data_y,s=1,c='C{}'.format(j),marker='s')
...
data = return_ePSF(manager,n_spec=nspec)
